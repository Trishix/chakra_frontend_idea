...
\hypersetup{colorlinks=true,linkcolor=teal,urlcolor=teal,pdftitle={CI Intel Feasibility and Idea Counter Questions}}
\pagestyle{fancy}\fancyhf{}\lhead{\textcolor{navy}{CI Intel}}\rhead{\textcolor{teal}{dot\_gitignore}}\cfoot{\thepage}\setlength{\headheight}{14pt}
...
\begin{document}
\begin{center}
{\Huge\bfseries\color{navy}CI Intel}\\[3pt]
...
\begin{enumerate}
\qa{What exact problem does CI Intel solve?}{CI Intel helps authorized investigators discover possible links between women-safety cases that are difficult to see across separate records. It organizes evidence by time, source and case, then presents a reviewable lead instead of making an accusation.}
...
\qa{Why focus on women-safety cases?}{Women-safety investigations can involve repeat patterns, shared contacts, unsafe locations and coordinated assistance. CI Intel is designed around those investigative needs while keeping survivor identity protected and avoiding automated guilt labels.}
\qa{What is the one-line value proposition?}{CI Intel turns fragmented, time-stamped case evidence into explainable cross-case leads that an authorized officer can verify, accept, reject or correct.}
\qa{What makes the idea different from a normal case-management system?}{A case-management system stores and searches records. CI Intel adds a temporal relationship layer that makes links between cases visible, shows the evidence path, and separates observed facts from inferred suggestions.}
...
\qa{What does CI Intel actually recommend?}{It recommends a case-link lead such as a shared masked phone, a transaction recipient or a repeated location pattern. The recommendation includes the relationship, confidence, source references and a route for human review.}
\qa{Does CI Intel identify a criminal?}{No. It identifies an investigative lead, never guilt. The interface labels observed evidence and AI-inferred relationships separately and requires an officer to review the source before a lead can appear in an auditable report.}
\end{enumerate}
...
\qa{What is the expected benefit to an investigator?}{CI Intel reduces time spent searching across records and gives the investigator a direct route from a graph relationship to the exact supporting source. It improves prioritization while leaving the investigative decision with the officer.}
\qa{How will you prove that the idea is not just a visual demo?}{The prototype uses deterministic fixtures, a defined graph schema, explicit evidence references, lead states and tests for state transitions. A next pilot would replay labeled cases and compare CI Intel's ranked leads with expert annotations.}
...
\qa{How does CI Intel protect survivors and witnesses?}{Names and sensitive fields can be masked by role, survivor identities can be restricted, exports are watermarked, and a supervisor approval is required for sensitive joins. The demo uses fictional records only.}
\qa{Could this system create bias against a community or language group?}{Yes, if source coverage or entity resolution is uneven. CI Intel therefore needs subgroup and language error analysis, human review, calibrated thresholds and a policy against using graph prominence as a proxy for culpability.}
...
\begin{enumerate}[resume]
\qa{Why should this be selected over a search engine or dashboard?}{Search returns records that contain a term; CI Intel connects entities and events across records, ranks a reviewable relationship and preserves the evidence path. It is designed for investigative reasoning rather than only retrieval.}
\qa{Why not let an LLM answer the investigator directly?}{An unconstrained LLM can invent links or lose provenance. CI Intel restricts the assistant to graph-backed evidence, displays source references and keeps the graph and document path as the primary verification surface.}
...
\qa{What if the most important clue is missing from the data?}{CI Intel cannot infer an absent fact reliably. It should show coverage limits, mark a lead as incomplete and allow an officer to add an authorized source rather than filling the gap with speculation.}
...
\end{enumerate}
\vfill
\begin{center}\small\color{teal}CI Intel is an evidence-led decision-support prototype. Every lead remains subject to officer verification.\end{center}
\end{document}
