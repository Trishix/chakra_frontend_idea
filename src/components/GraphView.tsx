...
function layoutPosition(entity:Entity, all:Entity[], compact=false):{x:number;y:number} {
 if(compact){const index=all.findIndex(item=>item.id===entity.id);return {x:index%2===0?18:198,y:36+index*128};}
...
export function GraphView({entities,relations,lead,onRelation,onEntity,expanded,onExpand}:{entities:Entity[];relations:Relation[];lead?:Lead;onRelation:(r:Relation)=>void;onEntity:(e:Entity)=>void;expanded:boolean;onExpand:()=>void}){
 ...
 const [compact,setCompact]=useState(()=>window.matchMedia('(max-width: 599px)').matches);
 ...
 const compactLayout=compact&&!expanded;
 useEffect(()=>{const media=window.matchMedia('(max-width: 599px)');const update=()=>setCompact(media.matches);media.addEventListener('change',update);return()=>media.removeEventListener('change',update);},[]);
 useEffect(()=>{setNodes(visible.map(e=>({id:e.id,type:'entity',position:layoutPosition(e,visible,compactLayout),data:{entity:e,active:activeIds.includes(e.id),dim:!!lead&&!activeIds.includes(e.id),bridge:e.caseIds.length>1}})));},[visible,lead,activeIds,compactLayout]);
